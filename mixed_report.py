...
df_normal = classifier_b(path='./assets/normal_dice/0/')
for i in range(1, 11):
    df_normal = pd.DataFrame.append(df_normal, classifier_b(path='./assets/normal_dice/'+str(i)+'/'))

df_normal['y_true'] = 0
df_normal['y_pred'] = np.where(df_normal['result'] == 'Normal', 0, 1)


df_anomalous = classifier_b(path='./assets/anomalous_dice/')
